...
export const Portada = ({ audioRef }) => {
  ...
  const backgroundRef = useRef(null); // Ref para el background-image-portada

  useEffect(() => {
    const audio = audioRef.current;
    
    // Obtener la altura del background cuando el componente se monta
    const calculateBackgroundHeight = () => {
      if (backgroundRef.current) {
        setBackgroundHeight(backgroundRef.current.getBoundingClientRect().height);
      }
    };

    calculateBackgroundHeight(); // Llamada inicial

    window.addEventListener('resize', calculateBackgroundHeight); // Actualizar al cambiar el tamaño de la ventana
  ...
  return (
    <div className="container-portada">
      <div className="background-image-portada" ref={backgroundRef}></div>
      <div className="gradient-overlay-portada"></div>
      <div className="content-bottom-portada" style={{ marginTop: backgroundHeight }}>
        <div className="texto-portada letra-peq">
          <span>11-10-2025</span>
        </div>
        <div className="texto-portada">
